...
**Lidl — детальный разбор буклета (2026-07-24)**, если решат вернуться к идее: `product_sitemap.xml.gz`
подтверждает ~69 товаров (непродовольственные акции — одежда/спорттовары/техника, не бакалея).
Буклет акций (`lidl.lv/l/lv/akciju-buklets/.../view/flyer/page/N`) рендерится через общую
CDN-платформу Schwarz Group `leaflets.schwarz` — чистые PNG страниц
(`imgproxy.leaflets.schwarz/.../page-NN_*.png`), без каких-либо встроенных данных (проверил DOM,
`window`-переменные, все сетевые запросы — ни JSON, ни hotspot-разметки). Значит нужен OCR/vision.
Макет **не единообразен**: часть страниц — чистая товарная сетка (название, вес, цена, числовой
код под товаром — похож на артикул, годится в `store_sku`), часть — купоны/промо-развороты без
цен на конкретные товары (напр. «-30% на категорию»), это ломает простой grid-by-coordinates OCR.
Обсуждали 3 варианта: (1) LLM vision per-страница — надёжнее всего, но платно и это ~45
страниц/неделя на один магазин; (2) классический Tesseract — бесплатно, но нужно отдельно
разруливать разные типы страниц, точность ниже; (3) отложить. **Выбрали (3), отложить** — если
вернутся к этой идее, начинать не с нуля, схема буклета уже понятна.
...
## Google Play Console (2026-07-25)

Подготовил [`PLAY_STORE_LISTING.md`](PLAY_STORE_LISTING.md) — тексты листинга на lv/ru/en,
...
## Индикатор изменения цены за неделю (2026-07-28)

Маленькая стрелка рядом с ценой на карточке товара (зелёная вниз/красная вверх) —
`supabase/migrations/0011_price_trend.sql` (**нужно накатить через SQL Editor, я сам не могу**),
создаёт view `store_product_price_trend` (последняя цена не позже 6 дней назад на каждый
store_product_id — price_history пишется при каждом прогоне скрапера независимо от того,
изменилась цена, поэтому просто "предыдущая строка" не подошла бы). На клиенте —
`lib/data/price_trend.dart` (`withPriceTrend()`, один batch-запрос на список карточек, не по
одному на карточку), подключено во всех репозиториях (`ProductRepository`, `WatchlistRepository`,
`ShoppingListRepository`) и в `ProductCard`/`MatchedProductCard` через новый
`widgets/price_trend_icon.dart`. Цвета — `colors.savings`/`colors.priceUp` уже были в
`app_theme.dart` (`priceUp` был объявлен, но нигде не использовался — похоже, эта фича
задумывалась ещё раньше). Живьём не проверено (нет вида в реальной БД без миграции), но
данные для неё есть — `price_history` копится с 2026-07-18, 556k+ строк на момент проверки.
Побочная находка при этой задаче: обнаружил случайную вторую копию `upload-keystore.jks` в
корне репозитория (не отслеживалась git, не коммитилась — удалил, причину появления не
установил, `.gitignore` на всякий случай расширил паттернами без привязки к папке).
