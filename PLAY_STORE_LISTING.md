...
1. **Политика конфиденциальности не готова к публикации.** `legal/PRIVACY_POLICY.md` — черновик
   с плейсхолдерами `[...]` (юрлицо, регистрационный номер, адрес, реальный email для privacy-
   вопросов) и без юридической проверки. Play Console **обязательно** требует публичный URL на
   неё в разделе App content → Privacy policy — просто файла в репозитории недостаточно, документ
   должен быть на хостинге по HTTPS. Заполнить плейсхолдеры и решить, где хостить (например,
   GitHub Pages — могу настроить, когда текст будет готов) — придётся сделать до подачи на
   ревью, не только до публикации.
