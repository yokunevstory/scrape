...
1. **Политика конфиденциальности не захостена.** Плейсхолдеры контролёра данных заполнены
   (2026-07-27): Jurijs Okunevs, физическое лицо без регистрации ИП, Латвия, контакт
   QuietBay@inbox.lv — см. `legal/PRIVACY_POLICY.md` и `legal/ACCOUNT_DELETION_POLICY.md`.
   Ещё без юридической проверки и с парой мелких операционных плейсхолдеров (сроки ответа на
   запрос удаления — сейчас `[10 рабочих дней]`/`[30 дней]`, это уже осознанный выбор срока,
   не идентификация). **Play Console обязательно требует публичный URL** на политику в разделе
   App content → Privacy policy — файла в репозитории недостаточно, нужен хостинг по HTTPS
   (например, GitHub Pages — могу настроить по запросу).
...
3. **Release-сборка подписывается debug-ключом** (`android/app/build.gradle.kts`:
   `signingConfig = signingConfigs.getByName("debug")`). Для Play Console нужен реальный
   upload-key (или Play App Signing) — это отдельная задача перед сборкой AAB для загрузки,
   не блокирует само создание записи приложения в консоли, но блокирует загрузку билда.
